        ...
        <div class="space-y-6 p-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription> Modifiez les détails du département et gérez les utilisateurs assignés. </CardDescription>
                </CardHeader>
                <CardContent>
                    <form @submit.prevent="submitForm" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-sm text-muted-foreground">Laissez vide si ce département est au plus haut niveau de la hiérarchie.</p>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2">
                                    <Input v-model="userSearch" placeholder="Rechercher un utilisateur..." class="w-[200px]">
                                        <template #leading>
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            ...
                                            <TableRow v-if="filteredUsers.length === 0">
                                                <TableCell colspan="5" class="h-24 text-center"> Aucun utilisateur trouvé. </TableCell>
                                            </TableRow>
                                        </TableBody>
                                    </Table>
                                </div>
...
import { ArrowLeftIcon, LoaderIcon, SearchIcon } from 'lucide-vue-next';
...
const breadcrumbs = [{ title: 'Départements', href: route('departments.index') }, { title: 'Modifier le département' }];
...
const transformDepartmentUsers = () => {
    ...
    return props.department.users.map((user) => {
        ...
        const isHead = user.pivot?.is_head === 1 || user.pivot?.is_head === true || user.pivot?.is_head === '1';
...
const allSelected = computed(() => {
    return filteredUsers.value.length > 0 && filteredUsers.value.every((user) => isUserSelected(user.id));
});
...
const toggleSelectAll = () => {
    if (allSelected.value) {
        ...
        form.users = form.users.filter((user) => !filteredUsers.value.some((filteredUser) => filteredUser.id === user.id));
    } else {
