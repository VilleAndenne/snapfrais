<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6">
            ...
            <!-- En-tête -->
            <div class="space-y-1">
                <h2 class="text-xl sm:text-2xl font-semibold tracking-tight">Créer un nouvel utilisateur</h2>
                <p class="text-sm text-muted-foreground">
                    Ajoutez un membre et rattachez-le à ses départements. Il recevra un e-mail pour définir son mot de passe.
                </p>
            </div>

            <form @submit.prevent="createUser" class="max-w-2xl space-y-6">
                <!-- Carte principale -->
                <div class="rounded-xl border bg-card shadow-sm overflow-hidden">
                    ...
                    <div class="flex items-center gap-3 sm:gap-4 border-b bg-muted/30 px-5 sm:px-8 py-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="font-medium truncate">{{ form.name || 'Nouvel utilisateur' }}</p>
...
const filteredDepartments = computed(() => {
    ...
    return props.departments.filter((dept) => dept.name.toLowerCase().includes(term));
});
