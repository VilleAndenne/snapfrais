<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6">
            <!-- En-tête -->
            <div class="space-y-1">
                <h2 class="text-xl sm:text-2xl font-semibold tracking-tight">Modifier le formulaire</h2>
                <p class="text-sm text-muted-foreground">
                    Mettez à jour le formulaire et configurez ses coûts (taux de remboursement et prérequis) un par un.
                </p>
            </div>
...
// Normalise un coût entrant en conservant les identifiants (maj vs création côté backend)
const normalizeCost = (cost) => ({
    ...
    reimbursement_rates: (cost.reimbursement_rates ?? []).map((rate) => ({
        ...
        value: rate.value ?? 0,
        transport: rate.transport ?? (cost.type === 'km' ? 'car' : null),
    })),
...
const form = useForm({
    ...
    costs: (props.form.costs ?? []).map(normalizeCost),
});
...
const addRequirement = () => {
    costDraft.value.requirements.push({ id: null, name: '', type: '' });
};
